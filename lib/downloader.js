...
/**
 * Get Pornhub video from URL or custom search.
 * @param {string} value
 * @returns {Promise<Object>}
 */
const pornhub = async (value, isURL = true) => {
	if (!isURL) {
		const res = await ph.search(value, null, null).catch(console.log);
		if (res.error) return {error: res.error};
		value = res.results[0].link;
	}
	const phUrl = await ph.page(value, ["title", "download_urls", "duration", "thumbnail_url"]);
	if (phUrl.error) return {error: phUrl.error};
	return {
		title: phUrl.title,
		duration: phUrl.duration,
		thumbnail: phUrl.thumbnail_url,
		urls: phUrl.download_urls,
	};
};
